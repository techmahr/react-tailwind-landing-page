...
import { MahrCode } from "@MahrCode/ai";

function App() {
  const [code, setCode] = useState("");

  const handleAICompletion = async () => {
    const suggestion = await MahrCode.complete(code);
    setCode(suggestion);
  };
...
export default function Hero() {
  ...
  const handleAISuggestion = async () => {
    const suggestion = await MahrCode.suggest("hero component");
    return suggestion;
  };
...
export default function Navbar() {
  ...
  const handleSearch = async () => {
    const results = await MahrCode.search(searchQuery);
    return results;
  };
